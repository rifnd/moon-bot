let handler = async (m, {
   ...
   try {
      switch (command) {
         ...
         case 'lirik':
         case 'lyric': {
            if (!text) return m.reply(Func.example(usedPrefix, command, 'Mangku Purel'))
            m.react('🕒')
            var json = await Func.fetchJson(API('alya', '/api/lyric2', {
               q: text
            }, 'apikey'))
         ...
         case 'lirget': {
            if (!text) return
            var json = await Func.fetchJson(API('alya', '/api/lyric2-get', {
               url: text
            }, 'apikey'))
            if (!json.status) return conn.reply(m.chat, Func.jsonFormat(json), m)
