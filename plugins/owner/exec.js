...
handler.before = async function (m, _2) {
   let { conn, store, args, groupMetadata, plugins, isOwner, Func, Scraper, setting } = _2
   ...
   if (m.text.startsWith('=>')) {
      try {
         ...
         let execFunc = new (async () => { }).constructor('print', 'm', 'handler', 'require', 'conn', 'store', 'Func', 'Scraper', 'setting', 'Array', 'process', 'args', 'groupMetadata', 'plugins', 'module', 'exports', 'argument', txt)
         _return = await execFunc.call(conn, (...args) => {
            if (--i < 1) return
            console.log(...args)
            return conn.reply(m.chat, format(...args), m)
         }, m, handler, require, conn, store, Func, Scraper, setting, CustomArray, process, args, groupMetadata, plugins, f, f.exports, [conn, _2])
      } catch (e) {
