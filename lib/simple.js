...
let tags = {
  album: 'Moon Music',
  APIC: fs.readFileSync('./src/moon-bot.jpg')
}
...
exports.makeWASocket = (connectionOptions, options = {}) => {
    ...
    return conn.generateMessage(
      jid,
      {
        ...
        contextInfo: {
          ...
          externalAdReply: {
            title: properties.title || global.set.wm,
            ...
            thumbnail: properties.thumbnail
              ? await Func.fetchBuffer(file)
              : await Func.fetchBuffer(global.set.thumbnail),
  ...
  conn.sendFile = async (
    ...
    let { status, file, filename, mime, size, extension } = await conn.getFile(
      url,
      name,
      opts && opts.referer ? opts.referer : false
    );
    //if (!status) return m.reply(`Failed`)
